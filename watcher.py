...
API_TOKEN = "7390264547:AAHxE2CAk-uhPlecG1ySwopVz6ZfX1Cryl0"
ADMIN_ID = "937173294"
MAX_RESTARTS = 5
    ...
    while True:
        if restart_count >= MAX_RESTARTS:
            ...
            if current_time - last_restart_time < RESTART_PERIOD:
                ...
                logging.warning(
                    "Maximum restart limit reached. Waiting for %.2f seconds...",
                    wait_time,
                )
                await notify_admin(
                    f"⚠️ Maximum restart limit reached. Waiting for {int(wait_time)} seconds before retrying."
                )
        ...
        await notify_admin(
            "⚠️ The bot has crashed and will be restarted in 10 seconds."
        )
        restart_count += 1
